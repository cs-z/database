                                        ...
                                        const Value& key, RID rid, page::EntryId index)
{
    ...
    return std::make_pair(new_page.get_page_id(),
...
static auto find_lower_entry(buffer::Pin<const PageT>& page, const Type& key_type, const Value& key)
{
    ...
    if constexpr (std::is_same_v<PageT, Leaf>)
    {
        if (index < page->get_entry_count() &&
            compare_keys(key_type, page->get_entry(index), key) == 0)
        {
    ...
    if constexpr (std::is_same_v<PageT, Inner>)
    {
        ...
        return entry_id > 0 ? page->get_entry_info(entry_id - 1)
                            : page->get_header().leftmost_child;
    }
    UNREACHABLE();
}
...
template <typename PageT>
static page::EntryId find_insert_entry(buffer::Pin<PageT>& page, const Type& key_type,
                                       const Value& key)
{
    const auto iter =
        std::upper_bound(page->cbegin(), page->cend(), key,
...
static std::pair<page ::Id, Value> insert_recursive(catalog::FileId file_id, page::Id page_id,
                                                    const Type& key_type, const Value& key, RID rid)
{
    ...
    const auto          child =
        index > 0 ? inner->get_entry_info(index - 1) : inner->get_header().leftmost_child;
...
Header> page, const Type &key_type, const Value &key, bool reverse)
{
    if (page->is_leaf) {
        buffer::Pin<const Leaf> leaf = std::move(page);
        const auto entry_id = reverse ? find_upper_entry(leaf, key_type, key) :
find_lower_entry(leaf, key_type, key); if (entry_id) { return std::make_pair(std::move(leaf),
*entry_id);
        }
        else {
            return {};
        ...
        buffer::Pin<const Inner> inner = std::move(page);
        const page::Id child = reverse ? find_upper_entry(inner, key_type, key) :
find_lower_entry(inner, key_type, key); return find_string_recursive(inner.shift(child), key_type,
key, reverse);
    }
}
...
row::read(key_type, inner->get_entry(page::EntryId { 0 })) : max_key)); for (page::EntryId i { 0 };
i < inner->get_entry_count(); i++) { depths.insert(debug_tree_recursive(key_type,
inner.shift(inner->get_entry_info(i)), i == 0 ? min_key : row::read(key_type, inner->get_entry(i -
1)), i + 1 == inner->get_entry_count() ? max_key : row::read(key_type, inner->get_entry(i + 1))));
        }
