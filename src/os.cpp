...
namespace os
{
	...
	static void file_read(int fd, page::Id page_id, void *buffer)
	{
		...
		if (static_cast<std::size_t>(bytes_returned) < bytes) {
			throw ServerError { "less bytes returned in read(" + std::to_string(fd) + "): " + std::to_string(bytes_returned) + " < " + std::to_string(bytes) };
		}
	}
	...
	static void file_write(int fd, page::Id page_id, const void *buffer)
	{
		...
		if (static_cast<std::size_t>(bytes_returned) < bytes) {
			throw ServerError { "less bytes returned in write(" + std::to_string(fd) + "): " + std::to_string(bytes_returned) + " < " + std::to_string(bytes) };
		}
	}
