...
namespace catalog
{
	...
	{
		const std::string statement = "SELECT NAME FROM " + TABLE_FILES.name + " WHERE ID = " + file_id.to_string();
	...
	{
		const std::string statement = "SELECT ID FROM " + TABLE_TABLES.name + " WHERE NAME = \'" + name + "\'";
		std::vector<Value> values = execute_internal_statement(statement);
		if (values.empty()) {
			return std::nullopt;
		}
